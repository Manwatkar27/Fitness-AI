...
import com.fitness.gateway.user.RegisteredRequest;
...
public class KeycloakUserSyncFilter implements WebFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        ...
        RegisteredRequest registeredRequest = getUserDetails(token);
        if (userId == null){
    ...
    private RegisteredRequest getUserDetails(String token) {
        try{
            String tokenWithoutBearer = token.replace("Bearer", "").trim();
            SignedJWT signedJWT = SignedJWT.parse(tokenWithoutBearer);
            JWTClaimsSet claims = signedJWT.getJWTClaimsSet();

            RegisteredRequest request = new RegisteredRequest();
